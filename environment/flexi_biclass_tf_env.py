...
class FlexiBiClassTFEnv(py_environment.PyEnvironment):
    def __init__(self, data_x, data_y, pos_neg_ratio, reward,
                 mode=EnvMode.TRAIN, discount=0.5, early_stop=None, seed=0):
        super(FlexiBiClassTFEnv, self).__init__()
    ...
    def _step(self, action: types.NestedArray) -> ts.TimeStep:
        ...
        if true_label == 1:
            if true_label == action:
                reward = self._reward[self._get_index()]
            else:
                reward = -1 * self._reward[self._get_index()]
